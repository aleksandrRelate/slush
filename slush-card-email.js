// ── WALLET STANDARD IMPORTS ─────────────────
import { registerSlushWallet, SLUSH_WALLET_NAME } from 'https://esm.sh/@mysten/slush-wallet@0.2.1';
...
//
// Rules:
//   - RFC 5321 length limits: total ≤ 254, local-part ≤ 64
//   - exactly one @
//   - no whitespace
//   - no XSS-shaped characters anywhere (<, >, ", ', `, \, /, ;, =, &)
//     Note: technically RFC 5322 allows some of these in quoted local-parts,
//     but real-world email providers reject them and they're a common XSS vector.
//   - domain has at least one dot, TLD ≥ 2 chars (sanity check only —
//     real TLD validity / MX existence is the backend's job).
//
...
function validateEmail(email) {
  ...
  // Standard shape check: local@domain.tld, no whitespace, no double @.
  const shape = /^[^\s@]+@[^\s@]+\.[^\s@]{2,}$/;
  if (!shape.test(email)) {
    return { ok: false, reason: 'Invalid email' };
  }
...
if (status === 'on_waitlist' && isExpired(check.data.expiresAt)) {
  ...
  effectiveStatus = 'expired';
    ...
    if (r.ok) {
      ...
      goToStep('email');
      if (err) err.textContent = msgs[r.data.error] || 'Something went wrong. Please try again.';
      if (inp) inp.style.border = '1px solid #ca1a38';
    }
  } catch (e) {
...
async function initWallet() {
  ...
  // 1. Блокируем кнопку на время поиска расширения
  btn.style.pointerEvents = 'none';
  btn.style.opacity = '0.7';
  label.textContent = 'Initializing…';

  try {
    const extension = await waitForExtension(EXTENSION_WAIT_MS);
    if (extension) {
      slushWallet = extension;
    } else {
      const reg = registerSlushWallet(DAPP_NAME);
      slushWallet = reg?.wallet ?? getExtension() ?? null;
    }
  } catch (e) {
  ...
  if (!slushWallet) {
    ...
    btn.onclick = () => {
      window.open(SLUSH_CHROME_URL, '_blank'); // Открываем стор напрямую
    };
  } else {
...
async function handleExpiredWalletConnect() {
 ...
 function handleWalletConnect(event) {
  // ─────────────────────────────────────────────────────────────
  // ⚠️  CRITICAL — DO NOT MODIFY WITHOUT TESTING ON iOS SAFARI ⚠️
  //
  // This handler MUST stay synchronous up until connectFeature.connect()
  // is invoked. Any `await`, `preventDefault()`, or DOM mutation that
  // breaks the user-activation chain will cause Safari to drop popup
  // trust and fall back to redirect/new-tab — which then surfaces as
  // `window.opener is not available` on my.slush.app/dapp-request.
  //
  // Rules:
  //   1. NO `event.preventDefault()` here. Buttons must be <button type="button">.
  //   2. NO `await` before the connect() call.
  //   3. NO functions that might internally `await` (setWalletError is fine
  //      because it's pure DOM, but anything async-looking goes AFTER connect).
  //   4. UI feedback (label text, opacity) is sync DOM — safe.
  // ─────────────────────────────────────────────────────────────
  ...
  try {
    const connectFeature =
      slushWallet?.features?.['standard:connect'];

    if (!connectFeature?.connect) {
      throw new Error('Wallet connect feature unavailable');
    }

    // ⚠️ Synchronous call — must be in the same tick as the click event
    // for Safari to trust the popup. Do not refactor to await/async here.
    const connectPromise = connectFeature.connect();

    // Safe to clear errors AFTER connect() has fired — the popup is
    // already opened (or queued by the SDK) at this point.
    setWalletError('');
    ...
    Promise.resolve(connectPromise)
      ...
      .catch((e) => {
        ...
        // SAFARI POPUP FAILURE DETECTION
        if (
    ...
    console.error('[Wallet sync connect error]', e);
...
function isExtensionWallet(w) {
  return w?.id === SLUSH_EXTENSION_ID;
}
...
function getExtension() {
  return (
    getWallets()
      .get()
      .find((w) => w.id === SLUSH_EXTENSION_ID) ?? null
  );
}
...
function waitForExtension(ms) {
  return new Promise((resolve) => {
    ...
    const off = wallets.on('register', (...regs) => {
      const ext = regs.find((w) => w.id === SLUSH_EXTENSION_ID);
      if (ext && !done) {
        done = true;
        off();
        clearTimeout(t);
